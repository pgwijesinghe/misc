...
df_26 = pd.read_excel(datafile, 'NEB_26')
df_38 = pd.read_excel(datafile, 'NEB_38')

# df, yran = df_26, [-13000,13000]
df, yran = df_38, [-11000,20000]
...
for biomass in biomass_list:  # loops through each biomass entry (main outer loop)
    ...
    kk = pd.DataFrame({'NEBpe_eq': a, "NEBel_eq": b, "NEBel": c},index=d)
    if biomass != "SRC Willow" and biomass != "Forest Residue":
    ...
    axes.set_ylabel("NEB $(MJ/t_{DM})$", fontsize=12)
    row += 1
