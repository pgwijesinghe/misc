...
for i in range(Npoints):
    psi[i] = np.exp(2.j*x[i]*(np.pi)/(2.*L))  
norm = get_norm(psi)
...
ani = animation.FuncAnimation(fig, animate, np.arange(1, 1620), init_func=init, interval=25, save_count=1620)
